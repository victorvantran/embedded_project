...
void LCD_Byte (unsigned char byte, char *str)       /* Send byte to LCD function */
{
    for (int i=0;i<8;i++)
    {
        str[i] = ((byte >> i) & 1) ? '1' : '0';     /* Convert unsigned char byte to array of bits (big endian result) */
    }
    
    for(int k=7;k>=0;k--)
	{
        LCD_Char(str[k]);                           /* Call LCD data write */
	}
}
